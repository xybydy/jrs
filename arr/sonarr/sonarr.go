...
import (
	"fmt"
	arr2 "github.com/xybydy/jrs/arr"
	"net/http"
	"net/url"
	"strconv"
	"strings"
...
func New(url, apiKey string) *Sonarr {
	s := new(Sonarr)
	s.URL = url
	s.ApiKey = apiKey
	s.Timeout = 60
...
func (s *Sonarr) Calendar(unmonitored bool, start, end string) (*http.Request, error) {
	data := url.Values{}
	...
	if req, err := s.Request(http.MethodGet, nil, "calendar"); err == nil {
...
func (s *Sonarr) GetOngoingCommands() (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodGet, nil, "command"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetCommandStatus(id string) (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodGet, nil, "command", id); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) RefreshSeries(id string) (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodPost, strings.NewReader(data.Encode()), "command"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) DiskSpace() (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodPost, nil, "diskspace"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetShow(seriesID string) (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodGet, strings.NewReader(data.Encode()), "episode"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetEpisode(episodeID string) (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodGet, nil, "episode", episodeID); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetEpisodeFiles(seriesID string) (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodGet, strings.NewReader(data.Encode()), "episodefile"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetShowFiles(id string) (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodGet, nil, "episodefile", id); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) DeleteEpisode(id string) (*http.Request, error) {
	...
	if req, err := s.Request("DELETE", nil, "episodefile", id); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) History(sortKey, page, pageSize, sortDir string) (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodGet, nil, "history"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) WantedMissing(sortKey, page, pageSize, sortDir string) (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodGet, nil, "wanted", "missing"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) Queue() (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodGet, nil, "queue"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) RemoveDownload(id string, blacklist bool) (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodGet, strings.NewReader(data.Encode()), "queue"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetProfiles() (*http.Request, error) {
	...
	if req, err := s.Request(http.MethodGet, nil, "profile"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) ConfigMediaManagement() (*http.Request, error) { // same
	if req, err := s.Request(http.MethodGet, nil, "config", "mediamanagement"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) ConfigNaming() (*http.Request, error) { // same
	if req, err := s.Request(http.MethodGet, nil, "config", "naming"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) ConfigIndexer() (*http.Request, error) { // same
	if req, err := s.Request(http.MethodGet, nil, "config", "indexer"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) ConfigDownloadClient() (*http.Request, error) { // same
	if req, err := s.Request(http.MethodGet, nil, "config", "downloadclient"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetDownloadClient() (*http.Request, error) { // same
	if req, err := s.Request(http.MethodGet, nil, "downloadclient"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) ConfigNotification() (*http.Request, error) { // same
	if req, err := s.Request(http.MethodGet, nil, "config", "notification"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) ConfigHost() (*http.Request, error) { // same
	if req, err := s.Request(http.MethodGet, nil, "config", "host"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) ConfigUI() (*http.Request, error) { // same
	if req, err := s.Request(http.MethodGet, nil, "config", "ui"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) ConfigNetImport() (*http.Request, error) {
	if req, err := s.Request(http.MethodGet, nil, "config", "netimport"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetNetImport() (*http.Request, error) {
	if req, err := s.Request(http.MethodGet, nil, "netimport"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) ConfigDelayProfile() (*http.Request, error) { // same
	if req, err := s.Request(http.MethodGet, nil, "delayprofile"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) ConfigQualityDefinition() (*http.Request, error) { // same
	if req, err := s.Request(http.MethodGet, nil, "qualitydefinition"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetIndexers() (*http.Request, error) { // same
	if req, err := s.Request(http.MethodGet, nil, "indexer"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetRemotePathMapping() (*http.Request, error) {
	if req, err := s.Request(http.MethodGet, nil, "remotePathMapping"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetRootFolder() (*http.Request, error) {
	if req, err := s.Request(http.MethodGet, nil, "rootfolder"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetExclusions() (*http.Request, error) {
	if req, err := s.Request(http.MethodGet, nil, "exclusions"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetMetadata() (*http.Request, error) {
	if req, err := s.Request(http.MethodGet, nil, "metadata"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) GetIndexerSchema() (*http.Request, error) {
	if req, err := s.Request(http.MethodGet, nil, "indexer", "schema"); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) DeleteIndexer(i arr2.IndexerSchema) (*http.Request, error) {
	id := fmt.Sprintf("%v", i.ID)
	if req, err := s.Request("DELETE", nil, "indexer", id); err == nil {
		return req, err
	} else {
		return nil, err
	}
...
func (s *Sonarr) SetIndexer(i arr2.IndexerSchema) (*http.Request, error) {
	if req, err := s.Request(http.MethodPost, nil, "indexer"); err == nil {
		return req, err
	} else {
		return nil, err
	}
