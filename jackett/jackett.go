...
func New(url, apiKey string) *Jackett {
	j := &Jackett{version: version, root: root, api: apiKey, url: url, headers: make(http.Header)}
...
func (j *Jackett) getAPIPath(category, action string) string {
	path := j.url + "/api/v" + j.version + "/" + category
...
func (j *Jackett) ExportTorznab(indexerID string, hostdomain string) string {
	if hostdomain == "" {
		return fmt.Sprintf(j.url + "/api/v" + j.version + "/indexers/" + indexerID + "/results/torznab/")
	} else {
...
func (j *Jackett) ExportPotato(indexerID string) string {
	return fmt.Sprintf(j.url + "api/v" + j.version + "/indexers/" + indexerID + "/results/potato/")
}
...
func (j *Jackett) updateIndexerConfig(indexerID, config string) *http.Request {
	...
	if req, err := http.NewRequest(http.MethodPost, path, strings.NewReader(config)); err == nil {
		req.Header = j.headers
		return req
	}
	return nil
...
func (j *Jackett) resultsForIndexer(indexerID, query string) *http.Request {
	...
	if req, err := http.NewRequest(http.MethodGet, path, strings.NewReader(query)); err == nil {
		req.Header = j.headers
		return req
	}
	return nil
...
func (j *Jackett) updateServerConfig(serverConfig string) *http.Request {
	path := j.getAPIPath("server", "config")
	if req, err := http.NewRequest(http.MethodPost, path, strings.NewReader(serverConfig)); err != nil {
		req.Header = j.headers
		return req
	}
	return nil
...
func (j *Jackett) updateAdminPassword(password string) *http.Request {
	...
	if req, err := http.NewRequest(http.MethodPost, path, strings.NewReader(password)); err == nil {
		req.Header = j.headers
		return req
	}
	return nil
